...
class PlayerErrorViewTestCase(BaseAPITestCase):
    ...
    def test_get_errors_something(self):
        ...
        # set an error
        PlayerError.objects.create(playlist_entry=self.pe1,
        ...
        # assert the response
        self.assertEqual(response.data['results'][0]['playlist_entry']['id'],
                         self.pe1.id)
        self.assertEqual(response.data['results'][0]['error_message'],
                         "dummy error")
