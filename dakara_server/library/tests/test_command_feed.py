...
from library.management.commands.feed_components.ffmpeg_wrapper import FFmpegWrapper
...
class FeedCommandTestCase(TestCase):
    ...
    def test_feed_command_rename(self):
        ...
        with TemporaryDirectory(prefix="dakara.") as dirpath:
            ...
            self.assertEqual(second_song.directory, "")
            self.assertEqual(
                second_song.title, os.path.splitext(second_file_filename)[0]
            )
            ...
            self.assertEqual(
                first_song_new.title, os.path.splitext(first_file_filename_new)[0]
            )
            ...
            self.assertEqual(
                second_song_new.title, os.path.splitext(second_file_filename)[0]
            )
    ...
    def test_feed_command_rename_different(self):
        ...
        with TemporaryDirectory(prefix="dakara.") as dirpath:
            ...
            self.assertEqual(
                first_song_old.title, os.path.splitext(first_file_filename)[0]
            )
    ...
    def test_feed_command_directory_change(self):
        ...
        with TemporaryDirectory(prefix="dakara.") as dirpath:
            ...
            first_file_filepath = self.create_media_file(
                os.path.join(dirpath, first_directory), first_file_filename
            )

            second_file_filename = "The second file.mp4"
            self.create_media_file(
                os.path.join(dirpath, first_directory), second_file_filename
            )

            # create 1 file in second directory
            third_file_filename = "The third file.mp4"
            self.create_media_file(
                os.path.join(dirpath, second_directory), third_file_filename
            )
            ...
            os.rename(
                first_file_filepath,
                os.path.join(dirpath, second_directory, first_file_filename),
            )
    ...
    def test_feed_command_rename_and_new(self):
        ...
        with TemporaryDirectory(prefix="dakara.") as dirpath:
            ...
            self.assertEqual(
                third_song_new.title, os.path.splitext(third_file_filename)[0]
            )

            # Check that old first file is still in database with the same id
            # and is now new first or third file
            self.assertTrue(
                first_song.id == first_song_new.id or first_song.id == third_song_new.id
            )
    ...
    def test_feed_command_with_lyrics(self):
        ...
        with TemporaryDirectory(prefix="dakara.") as dirpath:
            ...
            subtitle_file_filepath_origin = os.path.join(
                APP_DIR, RESSOURCES_DIR, SUBTITLES_DIR, "simple.ass"
            )

            shutil.copy(
                subtitle_file_filepath_origin,
    ...
    def test_feed_command_with_lyrics_txt(self):
        ...
        with TemporaryDirectory(prefix="dakara.") as dirpath:
            ...
            with open(os.path.join(dirpath, subtitle_file_filename), "wt") as file:
                file.write(lyrics)
    ...
    @unittest.skipUnless(FFmpegWrapper.is_available(), "FFmpeg is not available.")
    def test_feed_command_with_lyrics_embedded(self):
        ...
        with TemporaryDirectory(prefix="dakara.") as dirpath:
            ...
            media_file_filepath_origin = os.path.join(
                APP_DIR, RESSOURCES_DIR, "lyrics_embedded.mkv"
            )

            shutil.copy(
                media_file_filepath_origin, os.path.join(dirpath, media_file_filename)
            )
    ...
    def test_feed_command_rename_multiple(self):
        ...
        with TemporaryDirectory(prefix="dakara.") as dirpath:
            ...
            self.assertEqual(second_song_new.directory, "")
            self.assertEqual(
                second_song_new.title, os.path.splitext(second_file_filename_new)[0]
            )
    ...
    def test_feed_command_artist_work_tag_change(self):
        ...
        with TemporaryDirectory(prefix="dakara.") as dirpath:
            ...
            filename_new = (
                "The song name - New artist name - New work title - " "NEWTAG.mp4"
            )
    ...
    def test_feed_command_no_work_type(self):
        ...
        with TemporaryDirectory(prefix="dakara.") as dirpath:
            ...
            self.call_feed_command(
                dirpath, {"parser": "library/parser_test_no_work_type.py"}
            )
